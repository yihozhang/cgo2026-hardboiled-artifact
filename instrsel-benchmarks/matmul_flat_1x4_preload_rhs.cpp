...
bool matmul_bf16(Halide::Target target) {
    ...
    // preloading B
    B.in().compute_at(mm, rro).store_in(MemoryType::AMXTile).vectorize(_0).vectorize(_1);
    ...
    result.compile_to_lowered_stmt("/tmp/matmul_flat_1x4_preload_rhs.html", {A, B}, HTML, target);
